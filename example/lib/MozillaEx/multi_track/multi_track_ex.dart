    ...
    Widget makeBody() {
      return InAppWebView(
        ...
        onWebViewCreated: (controller) {
          // register a JavaScript handler with name "runHandler"
          controller.addJavaScriptHandler(
              handlerName: 'loadFile',
              callback: (args) async {
                // print arguments coming from the JavaScript side!
                String file = args[0];
                Tau.tau.logger.d(file);
                AudioBuffer b = await loadFile(file);
                if (b.isDisposed)
                  {
                    print('b is an invalid dart object');
                  }

                // return data to the JavaScript side!
                int index = audioBuffers.length;
                audioBuffers.add(b);
                return index;
              });
          ...
          controller.addJavaScriptHandler(
              ...
              callback: (args) async {
                ...
                // return data to the JavaScript side!
                return {'papa': 'papa_value', 'riri': 'riri_value'};
              });
        },
      );
    }
